...
pub fn replace_indent<'a>(src: &'a str, new_indent: &str) -> &'a str {
    ...
    let f1: Fn(&str) -> &str = get_add_function(&new_indent);
    let f2: Fn(&str) -> &str = get_cut_function(min_common_indent);

    // reindent(lines.iter(), exp_size, f1, f2)
    src
}

pub fn get_add_function<F>(indent: &str) -> F
    where F: Fn(&str) -> &str,
{
    |line: &str| format!("{}{}", indent, line)
    // if indent.is_empty() { |l| l } else { |l| indent + l }
}
pub fn get_cut_function<F>(min_common_indent: usize) -> F
    where F: Fn(&str) -> &str,
{
    |line: &str| { line[min_common_indent..] }
}
